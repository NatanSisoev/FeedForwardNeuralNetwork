...
void train_neural_net() {
    ...
    for (int it = 0; it < num_epochs; it++) {
        ...
        for (int i = 0; i < num_training_patterns; i++) {
            ...
            start_time = omp_get_wtime();
            feed_input(p);
            printf("input_feed:%.16f\n", omp_get_wtime() - start_time);
            start_time = omp_get_wtime();
            forward_prop();
            printf("forward_propagation:%.16f\n", omp_get_wtime() - start_time);
            start_time = omp_get_wtime();
            back_prop(p);
            printf("backward_propagation:%.16f\n", omp_get_wtime() - start_time);
            start_time = omp_get_wtime();
            update_weights();
            printf("weight_update:%.16f\n", omp_get_wtime() - start_time);
        }
    }
...
void test_nn() {
    ...
    for (int i = 0; i < num_test_patterns; i++) {
        ...
        start_time = omp_get_wtime();
        forward_prop();
        printf("predicting:%.16f\n", omp_get_wtime() - start_time);
        start_time = omp_get_wtime();
        printRecognized(i, lay[num_layers - 1]);
        printf("recognition:%.16f\n", omp_get_wtime() - start_time);
    }
